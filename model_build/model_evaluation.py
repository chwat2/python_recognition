...
def plot_curves(FP, FN, nb_samples, threshold_min, threshold_max, threshold_step):
    ...
    mplot.plot(np.arange(threshold_min, threshold_max + threshold_step, threshold_step), FP,
               label='Fałszywie pozytywne', color='tab:red')
    mplot.plot(np.arange(threshold_min, threshold_max + threshold_step, threshold_step), FN,
               label='Fałszywie negatywne', color='tab:green')
...
def evaluate_model(paths, test_files, train_labels, test_labels, frames, power, n_fft, hop_length, n_mels):
    ...
    mplot.hist(data[y_true == 0][:, 1], bins=bins, color='tab:blue', alpha=0.6, label='Sygnały oznaczone jako normalne',
               edgecolor='#FFFFFF')
    mplot.hist(data[y_true == 1][:, 1], bins=bins, color='tab:red', alpha=0.6, label='Sygnały oznaczone jako anomelie',
               edgecolor='#FFFFFF')
    ...
    plot_curves(FP, FN, nb_samples=df.shape[0], threshold_min=threshold_min, threshold_max=threshold_max,
